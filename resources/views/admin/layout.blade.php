...
<body class="h-full bg-slate-100 font-sans" x-data="{ sidebarOpen: false }">
    ...
    <div class="flex h-full">
                ...
                @foreach ($navItems as $item)
                    ...
                    <a href="{{ route($item['route']) }}"
                       ...
                       {{ $active ? 'bg-brand/15 text-brand border border-brand/25' : 'text-white/60 hover:text-white hover:bg-white/8' }}">
                        <svg class="w-4.5 h-4.5 shrink-0" fill="none" stroke="currentColor" stroke-width="1.75"
                            viewBox="0 0 24 24">
                        ...
                        @if ($item['route'] === 'admin.contacts.index')
                            ...
                            @if ($unread > 0)
                                <span
                                    class="ml-auto text-[10px] font-bold bg-red-500 text-white rounded-full px-1.5 py-0.5">{{ $unread }}</span>
                            @endif
                        @endif
                    </a>
                @endforeach
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            @if (session('success'))
                <div
                    class="mx-6 mt-4 flex items-center gap-3 bg-green-50 border border-green-200 text-green-700 rounded-xl px-4 py-3 text-sm">
