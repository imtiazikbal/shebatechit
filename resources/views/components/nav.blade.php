<header x-data="{ open: false, scrolled: false }" x-init="window.addEventListener('scroll', () => { scrolled = window.scrollY > 50 })" class="fixed top-0 inset-x-0 z-50">
    ...
    @php
        $navPhone    = isset($contactDetails['phone'])    ? $contactDetails['phone']->first()    : null;
        $navEmail    = isset($contactDetails['email'])    ? $contactDetails['email']->first()    : null;
        $navFacebook = isset($contactDetails['facebook']) ? $contactDetails['facebook']->first() : null;
        $navWhatsapp = isset($contactDetails['whatsapp']) ? $contactDetails['whatsapp']->first() : null;
        $navLinkedin = isset($contactDetails['linkedin']) ? $contactDetails['linkedin']->first() : null;
    @endphp
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            <div class="hidden md:flex items-center justify-between h-9 text-[11px] text-white/45">
                ...
                <div class="flex items-center gap-5">
                    @if($navPhone)
                    <a href="tel:{{ preg_replace('/\s+/', '', $navPhone->value) }}" class="flex items-center gap-1.5 hover:text-brand transition-colors">
                        <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                        {{ $navPhone->value }}
                    </a>
                    @endif
                    @if($navPhone && $navEmail)<span class="h-3 w-px bg-white/10"></span>@endif
                    @if($navEmail)
                    <a href="mailto:{{ $navEmail->value }}" class="flex items-center gap-1.5 hover:text-brand transition-colors">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-2.5">
                        @if($navFacebook)
                        <a href="{{ $navFacebook->value }}" target="_blank" aria-label="Facebook" class="hover:text-brand transition-colors">
                        ...
                        @endif
                        @if($navWhatsapp)
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $navWhatsapp->value) }}" target="_blank" aria-label="WhatsApp" class="hover:text-brand transition-colors">
                        ...
                        @endif
                        @if($navLinkedin)
                        <a href="{{ $navLinkedin->value }}" target="_blank" aria-label="LinkedIn" class="hover:text-brand transition-colors">
